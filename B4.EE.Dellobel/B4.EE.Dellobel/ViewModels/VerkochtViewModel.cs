...
{
    public class VerkochtViewModel : FreshBasePageModel
    {
        ...
        public VerkochtViewModel(IKlantenService klantenService, IVoertuigenService voertuigenService)
        {
        ...
        private ObservableCollection<Auto> voertuigenVerkocht;
        public ObservableCollection<Auto> VoertuigenVerkocht
        {
            get { return voertuigenVerkocht; }
            set
            {
                voertuigenVerkocht = value;
                RaisePropertyChanged(nameof(VoertuigenVerkocht));
            }
        }
        ...
        {
            var voertuigen = voertuigenVerkocht;
            //VoertuigenAanbod = null;
            VoertuigenVerkocht = new ObservableCollection<Auto>(voertuigen);

            foreach (Auto a in VoertuigenVerkocht)
            {
                Merk = a.Merk;
                Model = a.Model;
        ...
        public ICommand ToEditVerkochtCommand => new Command<Auto>(
          async (Auto auto) =>
          {
              editAuto = auto;

              await CoreMethods.PushPageModelWithNewNavigation<EditVerkochtViewModel>(editAuto, true);
